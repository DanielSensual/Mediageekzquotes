...
/* ═══════════════════════════════════════════════════════════════
   Custom Square Checkout — Alta Vida (Jesse Kader)
   Uses client's Square account credentials.
   Supports: Credit/Debit Card, Apple Pay, Google Pay
   ═══════════════════════════════════════════════════════════════ */

const APP_ID = 'sq0idp-24Bx6Sw25pmV9YJw_IN_Gw';
const LOCATION_ID = '5BF9GG8J1JQNH';

function CheckoutContent() {
    const searchParams = useSearchParams();
    const amount = parseFloat(searchParams.get('amount') || '0');
    const desc = searchParams.get('desc') || 'MediaGeekz — Alta Vida Event Coverage';
    ...
    const processPayment = useCallback(async (sourceId) => {
        ...
        setError(null);
        try {
            const res = await fetch('/api/alta-vida-payment', {
                method: 'POST',
                ...
                <div className="checkout-card">
                    ...
                    {success ? (
                        <div className="checkout-success">
                            ...
                            <a href="/altavidaevent" className="back-link">
                                ← Return to Invoice
                            </a>
                        </div>
                    ) : (
                        <>
                            ...
                            <a href="/proposals/alta-vida-pool-party" className="back-link">
                                ← Back to Invoice
                            </a>
                        </>
                    )}
...
export default function AltaVidaCheckoutPage() {
    return (
        <Suspense fallback={
